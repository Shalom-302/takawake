...
def create_tables():
    """Cette fonction ne crée plus les tables car elles sont gérées par Alembic."""

    # Les tables sont désormais gérées par les migrations Alembic
    logger.info("Les tables de paiement sont gérées par les migrations Alembic")
...
def init_db_tables(engine: Any, session: Session) -> None:
    ...
    # Les tables sont désormais gérées par les migrations Alembic
    logger.info("Les tables de paiement sont gérées par les migrations Alembic")
    # Référence aux tables pour information seulement :
    # tables = [
    #    PaymentDB.__table__,
