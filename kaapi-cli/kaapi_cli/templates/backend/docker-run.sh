...
case "$COMMAND" in
    ...
    db)
        ...
        $COMPOSE_CMD exec db psql -U postgres -d kaapi
        ;;
    redis)
    ...
    dev)
        ...
        if [ "$MONITORING" = true ]; then
            $COMPOSE_CMD up -d db redis celery prometheus grafana loki alertmanager promtail
            $COMPOSE_CMD up api
        else
            $COMPOSE_CMD up -d db redis celery
            $COMPOSE_CMD up api
        fi
        ;;
    *)
