...
class ApiGatewayMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if self.config.enable_audit_logging:
            audit_log = ApiAuditLogDB.start_request_log(
                request=request,
                sensitive_headers=self.config.sensitive_headers,
                sensitive_body_fields=self.config.sensitive_body_fields
            )
        ...
        try:
            ...
            if audit_log and self.config.enable_audit_logging:
                ...
                ApiAuditLogDB.complete_request_log(
                    audit_log,
